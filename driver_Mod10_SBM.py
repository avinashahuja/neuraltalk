...
def main(params):
    ...
    for it in xrange(max_iters):
        ...
        # TODO: implement without replacement if possible
        batch = [dp.sampleImageSentencePair() for _ in xrange(batch_size)]

        for x in batch:
            x['sentence']['tokens'] = [w for w in x['sentence']['tokens'] if w in wordtoix]

        #batch1 = [[wordtoix[w] for w in x['sentence']['tokens'] if w in wordtoix] for x in batch]

        batch.sort(key=lambda x: len(x['sentence']['tokens']), reverse=True)

        gtix = -np.ones((batch_size, max_sentence_length), dtype=np.int32)
        ...
        for i,x in enumerate(batch):
            gtix[i][len(x['sentence']['tokens'])] = 0
        lstm.reset_state()
        lstm.zerograds()
        ...
        loss, out = lstm(input, False, True, True, output)
        for j in xrange(max_sentence_length):
            if j < max_sentence_length - 1:
                input = gtix[ gtix[:,j] > 0, j]
                output = gtix[gtix[:,j] > 0, j + 1]
                if len(input) > 0:
		    if GPU_id != -1:
                        input = cuda.to_gpu(input, GPU_id)
                        output = cuda.to_gpu(output, GPU_id)
                    l, out = lstm(input, True, True, True, output)
                    loss += l
            else:
                input = gtix[gtix[:,j] > 0, j]
                output = np.zeros(input.size, dtype=np.int32)
                if len(input) > 0:
		    if GPU_id != -1:
                        input = cuda.to_gpu(input, GPU_id)
                        output = cuda.to_gpu(output, GPU_id)
	...
	loss.unchain_backward()
        ...
        if (((it + 1) % eval_period_in_iters) == 0 and it < max_iters - 5) or is_last_iter:
            val_ppl = eval_perplexity('val', dp, lstm, max_sentence_length, params, misc)  # perform the evaluation on VAL set
	    ...
	    serializers.save_hdf5('lstm_models/lstm_model_%s_%d.model' % (job_id, it), lstm)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-a', '--do_grad_check', dest='do_grad_check', type=int, default=0,
                        help='perform gradcheck? program will block for visual inspection and will need manual user input')
    parser.add_argument('--fappend', dest='fappend', type=str, default='baseline',
                        help='append this string to checkpoint filenames')
    parser.add_argument('-o', '--checkpoint_output_directory', dest='checkpoint_output_directory', type=str,
                        default='cv/', help='output directory to write checkpoints to')
    parser.add_argument('--worker_status_output_directory', dest='worker_status_output_directory', type=str,
                        default='status/', help='directory to write worker status JSON blobs to')
    parser.add_argument('--write_checkpoint_ppl_threshold', dest='write_checkpoint_ppl_threshold', type=float,
                        default=-1, help='ppl threshold above which we dont bother writing a checkpoint to save space')
    parser.add_argument('--init_model_from', dest='init_model_from', type=str, default='',
    ...
    parser.add_argument('--image_encoding_size', dest='image_encoding_size', type=int, default=256,
                        help='size of the image encoding')
    parser.add_argument('--word_encoding_size', dest='word_encoding_size', type=int, default=256,
                        help='size of word encoding')
    parser.add_argument('--hidden_size', dest='hidden_size', type=int, default=1024,
    ...
    # rnn-specific params
    parser.add_argument('--rnn_relu_encoders', dest='rnn_relu_encoders', type=int, default=0,
                        help='relu encoders before going to RNN?')
    parser.add_argument('--rnn_feed_once', dest='rnn_feed_once', type=int, default=0,
    ...
    parser.add_argument('-m', '--max_epochs', dest='max_epochs', type=int, default=50,
    ...
    parser.add_argument('--decay_rate', dest='decay_rate', type=float, default=0.999,
    ...
    parser.add_argument('--word_count_threshold', dest='word_count_threshold', type=int, default=5,
                        help='if a word occurs less than this number of times in training data, it is discarded')
    ...
    parser.add_argument('--eval_batch_size', dest='eval_batch_size', type=int, default=100,
                        help='for faster validation performance evaluation, what batch size to use on val img/sentences?')
    ...
    parser.add_argument('-g','--GPU', dest='GPU', type=int, default=0,
